...
def find_facets(atoms,facet=0,mirror=0,offset=[2.0,0.0,0.0],angle=0):
  ...
  Rmat=[[1.0, 0, 0],[0.0, cos_theta, sin_theta],[ 0.0, -sin_theta, cos_theta]]
...
print("--- Area "+str(largest_AA)+" : "+str(AA)+" --- Area "+str(largest_AB)+" : "+str(AB) )


for i in range(12):
  ang=i*15
  off=[rx, 0., 0.] 
  atomsAout,tmp,AA=find_facets(atomsA,facet=largest_AA,angle=ang,offset=off)
  atomsBout,tmp,AB=find_facets(atomsB,facet=largest_AB,mirror=1)
  atomsBout.extend(atomsAout)
  traj.write(atomsBout)
  for j in range(12):
    ang_disp=j*30
    off=[rx, ryz*math.cos(ang_disp*math.pi/180.),ryz*math.sin(ang_disp*math.pi/180. )]
    atomsAout,tmp,AA=find_facets(atomsA,facet=largest_AA,angle=ang,offset=off)
    atomsBout,tmp,AB=find_facets(atomsB,facet=largest_AB,mirror=1)
    atomsBout.extend(atomsAout)
    traj.write(atomsBout)
